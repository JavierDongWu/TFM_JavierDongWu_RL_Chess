...
def train_action_mask(env_fn, steps=10_000, seed=0, saved_model = None, **env_kwargs):
    ...
    if saved_model is None:    
        model = MaskablePPO(MaskableActorCriticPolicy, env, verbose=1, learning_rate=gv.learning_rate, ent_coef = gv.ent_coef, tensorboard_log=gv.tensorboard_log)
        #model = MaskablePPO(MaskableActorCriticPolicy, env, verbose=1, learning_rate=0.001, ent_coef = 0.01)
    else:
        model = saved_model
        model.set_env(env)
