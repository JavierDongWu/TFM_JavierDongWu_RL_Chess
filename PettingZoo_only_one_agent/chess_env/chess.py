...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, render_mode: str | None = None, screen_height: int | None = 800, logger = False, evaluate = False, opponent_model = None, num_previous_boards = 3):
        ...
        #In this case there will be only one agent, playing against an opponent
        ...
        self.agents = [f"player_{i}" for i in range(self.number_of_agents)]
        ...
        #self.stockfish = Stockfish(path="stockfish", depth=4, parameters={"Threads":6, "Hash": 512})
        #The one with depth 1 will be used as opponent
        #self.stockfish_1 = Stockfish(path="stockfish", depth=1, parameters={"UCI_LimitStrength": True, "UCI_Elo": 1350})
        #self.previous_evaluation = self.evaluation_stockfish(self.board)
        #self.minmax_agent = minmax_agent.MinMaxAlphaBetaIncrementalEvaluationAgent(depth=1)
    ...
    #Observe funtion changed to be used from the black perspective
    ...
    #Function used to obtain the white observation, needed for the model who plays as white
    def observe_white(self):
        #current_index = self.possible_agents.index(agent)
        current_index = 0
    ...
    def reset_orig(self, seed=None, options=None):
        ...
        # random_white_move = random.choice(list(self.board.legal_moves))
        # self.board.push(random_white_move)
        if self.opponent_model is None:
            white_move = random.choice(list(self.board.legal_moves))
            # white_move = self.predict_capture_move(self.board)
        else:
            try:
                observed_board = self.observe_white()
                observation = observed_board["observation"]
                action_mask = observed_board["action_mask"]        

                act = int(
                    self.opponent_model.predict(
                        observation, action_masks=action_mask, deterministic=False # Deterministic to false for faster performance
                    )[0]
                )
                action = int(act)
        
                l = chess_utils.legal_moves(self.board)
                #black_move = chess_utils.action_to_move(self.board, action, current_index)
                white_move = chess_utils.action_to_move(self.board, action, 0) #For the white player the index is 0
                assert white_move in self.board.legal_moves
            except:
                #Sometimes action to move function gives error with deterministic=False
                print("white move exception, a random move will be used")
                white_move = random.choice(list(self.board.legal_moves))
    ...
    def reset(self, seed=None, options=None):
        ...
        for i in range(random_number):
            ...
            random_black_move = random.choice(list(self.board.legal_moves))
            self.board.push(random_black_move)
    ...
    def set_game_result(self, result_val):
        for i, name in enumerate(self.agents):
            ...
            coef_number = -1
            #coef_number = 1 * max(1, (96 - self.board.fullmove_number) / 32)
    ...
    def step(self, action):
        ...
        current_index = 1 #As the agent only plays as black agent
        ...
        if not game_over:
            #Save the board
            next_board = chess_utils.get_observation(self.board.copy(), player=1)
            self.board_history = np.dstack(
                (next_board[:, :, 7:], self.board_history[:, :, :-13])
            )    

            #black_move = random.choice(list(self.board.legal_moves))
            #black_move = self.predict_capture_move(self.board)
            #black_move = self.minmax_agent.predict_move(self.board)
            #self.stockfish_1.set_fen_position(self.board.fen())
            #black_move = chess.Move.from_uci(self.stockfish_1.get_best_move())

            if self.opponent_model is None:
                white_move = random.choice(list(self.board.legal_moves))
                # white_move = self.predict_capture_move(self.board)
            else:
                #Use a ppo model to predict the white player moves
                try:
                    observed_board = self.observe_white()
                    observation = observed_board["observation"]
                    action_mask = observed_board["action_mask"]        
                    ...
                    act = int(
                        self.opponent_model.predict(
                            observation, action_masks=action_mask, deterministic=False # Deterministic to false to increase performance
                        )[0]
                    )
                    action = int(act)
            
                    l = chess_utils.legal_moves(self.board)
                    white_move = chess_utils.action_to_move(self.board, action, 0) #For the black player the index is 1
                    assert white_move in self.board.legal_moves
                except:
                    #Sometimes action to move function gives error with deterministic=False
                    print("white move exception, a random move will be used")
            ...
            if board_value_change != 0:
                reward_given_board = round(-0.03 * board_value_change, 7)
                self.rewards[self.agents[0]] = reward_given_board
        ...
        # We always take the perspective of the black agent
        next_board = chess_utils.get_observation(self.board, player=1)
        self.board_history = np.dstack(
            (next_board[:, :, 7:], self.board_history[:, :, :-13])
        )
    ...
    #Function used to obtain the board evaluation from stockfish
    def evaluation_stockfish(self, board):
        ...
        if self.stockfish.is_fen_valid(board_fen):
            ...
            evaluation = {"type": "cp", "value": -10001}
    ...
    #Returns a capture move if possible, if not, a random move
    def predict_capture_move(self, board):
