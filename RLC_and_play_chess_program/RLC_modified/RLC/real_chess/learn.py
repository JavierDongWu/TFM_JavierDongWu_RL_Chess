import numpy as np
import time
from RLC_modified.RLC.real_chess.tree import Node
import chess_agents
import math
import gc
    ...
    def play_game(self, k, maxiter=80):
        ...
        while not episode_end:
            ...
            # Black's turn is myopic
                ...
                if isinstance(self.env.opposing_agent, chess_agents.MinMaxAlphaBetaIncrementalEvaluationAgent):
                    max_move = self.env.opposing_agent.predict_move(self.env.board)
                else:
                    max_move = None
                    max_value = np.NINF
            ...
            error = np.float64(np.squeeze(error))
