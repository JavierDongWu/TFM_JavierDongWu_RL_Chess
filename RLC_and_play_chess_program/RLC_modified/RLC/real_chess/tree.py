    ...
    def simulate(self, model, env, depth=0, max_depth=4, random=False, temperature=1):
        ...
        if env.board.turn and random:
            ...
            for move in env.board.generate_legal_moves():
                if isinstance(env.opposing_agent, chess_agents.MinMaxAlphaBetaIncrementalEvaluationAgent):
                    board_for_minmax = env.board
                ...
                if (result == "1-0" and env.board.turn) or (
                    ...
                    if env.board.turn:
                        ...
                        if isinstance(env.opposing_agent, chess_agents.MinMaxAlphaBetaIncrementalEvaluationAgent):
                            sucval = np.squeeze(env.opposing_agent.predict_state(board_for_minmax, move))
                        else:
