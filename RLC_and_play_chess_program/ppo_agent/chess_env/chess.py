...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, render_mode: str | None = None, screen_height: int | None = 800, logger = False, evaluate = False, previous_boards = 3):
        EzPickle.__init__(self, render_mode, screen_height)
        super().__init__()
        
        #SNumber of previous boards, it affects the obsercation size
        self.NumPreviousBoards = previous_boards
        #In this case, training will be done with only one agent
        ...
        self.stockfish = Stockfish(path="stockfish", depth=2)
    ...
    #Function used to obtain the board evaluation from stockfish, for the white player
    def evaluation_stockfish(self, board):
        ...
        if self.stockfish.is_fen_valid(board_fen):
            ...
            evaluation = {"type": "cp", "value": -2001}

        if evaluation["type"] == "cp":
            eval = evaluation["value"]
        else:
            if evaluation["value"] != 0:
                eval = 2000/evaluation["value"]
            else:
