...
from ppo_agent.chess_env import chess
import random
...
class SB3ActionMaskWrapper(pettingzoo.utils.BaseWrapper):
    ...
    def step(self, action):
        ...
        #La partida acaba siempre después de que se devuelva el reward del jugador que ha perdido
        # if self.FinalResult != 2 and not self.loser_finished:
        #     termination = False
        #     truncation = False
        #     info = {name: {"legal_moves": [self.move_null]} for name in self.agents}
        # else:
...
def train_action_mask(env_fn, steps=10_000, seed=0, saved_model = None, **env_kwargs):
    ...
    if saved_model is None:    
        model = MaskablePPO(MaskableActorCriticPolicy, env, verbose=1, learning_rate=0.0005, ent_coef = 0.01, tensorboard_log="./log/MASKPPO")
    else:
        model = saved_model
        model.set_env(env)
...
def eval_action_mask(env_fn, num_games=100, render_mode=None, **env_kwargs):
    ...
    try:
        latest_policy = max(
            ...
            #glob.glob("tfm.zip"), key=os.path.getctime
        )
    except ValueError:
...
if __name__ == "__main__":
    ...
    only_see_model_play = False
    num_step_to_train = 5_000_000
    use_saved_model = True
    use_logger = False #INFO or ERROR level

    env_kwargs = {"logger": use_logger, "evaluate": only_evaluate_model}
    ...
    if not only_see_model_play and not only_evaluate_model:
        ...
        if(use_saved_model):
                ...
                train_action_mask(env_fn, steps=num_step_to_train, seed=1,saved_model = model, **env_kwargs )
        else:
            train_action_mask(env_fn, steps=num_step_to_train, seed=1, saved_model = None, **env_kwargs)

        env_kwargs = {"logger": use_logger, "evaluate": True}
